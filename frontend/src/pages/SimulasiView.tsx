...
import { ArrowDown, ArrowUp, CalendarDays, CheckCircle2, CircleDot, ClipboardCheck, Copy, Download, ExternalLink, FileText, FileUp, GripVertical, Grid2X2, ImageIcon, KeyRound, Link2, ListChecks, ListOrdered, Plus, Save, Send, Star, Table2, TextCursorInput, Trash2, UsersRound, Wifi, WifiOff, X } from 'lucide-react'
...
type QuestionType = 'pg_tunggal' | 'pg_kompleks' | 'benar_salah' | 'menjodohkan' | 'isian_singkat' | 'uraian' | 'dropdown' | 'skala_linear' | 'rating' | 'kisi_pg' | 'kisi_checkbox' | 'tanggal' | 'waktu' | 'susun_urutan' | 'unggah_berkas'
...
function AnswerBuilder({ tipe, config, setConfig, groupName = 'visual-correct-choice' }: { tipe: QuestionType; config: any; setConfig: (config: any) => void; groupName?: string }) {
  if (tipe === 'pg_tunggal' || tipe === 'pg_kompleks' || tipe === 'dropdown') return <ChoiceBuilder config={config} multiple={tipe === 'pg_kompleks'} setConfig={setConfig} groupName={groupName} />
  ...
  if (tipe === 'tanggal' || tipe === 'waktu') return <ShortAnswerBuilder config={config} setConfig={setConfig} inputType={tipe === 'tanggal' ? 'date' : 'time'} />
  if (tipe === 'skala_linear' || tipe === 'rating') return <ScaleBuilder config={config} setConfig={setConfig} rating={tipe === 'rating'} groupName={groupName} />
  if (tipe === 'kisi_pg' || tipe === 'kisi_checkbox') return <GridBuilder config={config} setConfig={setConfig} multiple={tipe === 'kisi_checkbox'} />
  if (tipe === 'susun_urutan') return <OrderBuilder config={config} setConfig={setConfig} />
  if (tipe === 'unggah_berkas') return <FileUploadBuilder config={config} setConfig={setConfig} />
...
function ShortAnswerBuilder({ config, setConfig, inputType = 'text' }: { config: any; setConfig: (config: any) => void; inputType?: string }) { const answers = config.acceptedAnswers || []; const update = (acceptedAnswers: string[]) => setConfig({ ...config, acceptedAnswers }); return <div className="space-y-2"><div className="flex items-center justify-between"><div><h4 className="font-medium">Jawaban yang diterima</h4><p className="text-xs text-muted-foreground">{inputType === 'date' ? 'Masukkan tanggal jawaban dengan format kalender.' : inputType === 'time' ? 'Masukkan waktu jawaban dengan format jam dan menit.' : 'Masukkan variasi jawaban yang tetap dinilai benar, misalnya “empat” dan “4”.'}</p></div><Button type="button" size="sm" variant="outline" onClick={() => update([...answers, ''])}><Plus className="h-3.5 w-3.5" /> Tambah</Button></div>{answers.map((answer: string, index: number) => <div key={`${index}-${answer}`} className="flex gap-2"><Input type={inputType} value={answer} onChange={(event) => update(answers.map((item: string, itemIndex: number) => itemIndex === index ? event.target.value : item))} aria-label={`Jawaban diterima ${index + 1}`} /><Button type="button" size="icon" variant="ghost" disabled={answers.length <= 1} onClick={() => update(answers.filter((_: string, itemIndex: number) => itemIndex !== index))}><Trash2 className="h-4 w-4" /></Button></div>)}</div> }
...
function BuilderCanvas({ token, students, classes, bank, materials, initialPaket, onClose }: any) {
  ...
  const addQuestion = (type: QuestionType, at = draft.items.length) => { setDraft((valueNow: any) => { const items = [...valueNow.items]; items.splice(Math.min(at, items.length), 0, { soal: canvasQuestion(type), bobot: 1 }); return { ...valueNow, items } }); setActive(at) }
...
function StudentQuestionPreview({ question, token }: { question: Row; token: string }) {
  ...
  return <Card className="border-primary/30 bg-primary/[0.02] p-5">
    ...
    <div className="mt-4"><QuestionAnswerControl question={question} questionId={questionId} value={value} onChange={setValue} /></div>
...
function PackageDetail({ packet, token, items, questions, classes, students, assigned, setAssigned, results, shareTokens, detail, writable, canGrade, onAttach, onAssign, onPublish, onDuplicate, onInspect, onGrade, onCreateShare, onRevokeShare, onPreview, onExport, onExportXlsx }: any) {
  return <div className="space-y-4">
