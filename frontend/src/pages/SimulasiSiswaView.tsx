...
import { AlertCircle, ArrowLeft, ArrowRight, CheckCircle2, Clock3, Flag, Grid2X2, Info, Link2, LogOut, Play, Save, Send, Wifi, WifiOff } from 'lucide-react'
...
function answered(question: Row, value: any) {
  if (value === null || value === undefined || value === '' || value === 'null') return false
  const snapshot = question.soal || question
  const type = String(snapshot.tipe || '')
  const config = snapshot.konfigurasi || {}
  if (type === 'unggah_berkas') return Array.isArray(value) && value.length > 0
  if (type === 'susun_urutan') return Array.isArray(value) && value.length === (config.choices || []).length && value.length > 0
  if (type === 'kisi_pg' || type === 'kisi_checkbox') {
    const rows = config.rows || []
    if (!rows.length || typeof value !== 'object' || Array.isArray(value)) return false
    return rows.every((row: Row) => type === 'kisi_checkbox'
      ? Array.isArray(value[row.id]) && value[row.id].length > 0
      : typeof value[row.id] === 'string' && value[row.id].length > 0)
  }
  if (type === 'benar_salah') return (config.statements || []).length > 0 && typeof value === 'object' && (config.statements || []).every((row: Row) => typeof value[row.id] === 'boolean')
  if (type === 'menjodohkan') return (config.left || []).length > 0 && typeof value === 'object' && (config.left || []).every((row: Row) => typeof value[row.id] === 'string' && value[row.id].length > 0)
  if (type === 'pg_tunggal' || type === 'dropdown' || type === 'isian_singkat' || type === 'uraian' || type === 'tanggal' || type === 'waktu') return typeof value === 'string' && value.trim().length > 0
  if (type === 'skala_linear' || type === 'rating') return typeof value === 'number' && Number.isFinite(value)
  if (type === 'pg_kompleks') return Array.isArray(value) && value.length > 0
...
function clearPersistedIdempotencyKey(storageKey: string) {
  try { window.sessionStorage.removeItem(storageKey) } catch { /* session storage may be unavailable in hardened browser contexts */ }
}
type PendingAnswer = { value: any; sequence: number; idempotencyKey?: string }
...
export function SimulasiSiswaView({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  const sharedRequest = (path: string, method = 'GET', body?: unknown, extraHeaders?: Record<string, string>) => request(path, token, method, body, undefined, { ...sharedHeaders, ...extraHeaders })
  ...
  const themeColor = workspace?.paket?.temaWarna || instruction?.paket?.temaWarna || result?.temaWarna || rows[0]?.paket?.temaWarna || '#1c5d94'
  const activeAttemptRef = useRef('')
  const activeAttemptIsEditingRef = useRef(false)
  ...
  }, [themeColor, workspace, instruction, result])
  ...
  useEffect(() => { if (!workspace || workspace.upaya?.status !== 'berlangsung' || remaining <= 0) return; const timer = window.setInterval(() => setRemaining((value) => Math.max(0, value - 1)), 1000); return () => window.clearInterval(timer) }, [workspace, remaining])
  useEffect(() => { if (!workspace || workspace.upaya?.status !== 'berlangsung' || remaining !== 0) return; void submit(true, true) }, [remaining, workspace]) // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (!workspace) return
  ...
  async function openResult(attemptId: string) { try { setResult(await sharedRequest(`/simulasi/saya/upaya/${attemptId}/hasil`)); setWorkspace(null); setInstruction(null) } catch (error: any) { toast.error(error.message || 'Hasil simulasi tidak dapat dibuka.') } }
  ...
  async function openWorkspace(id: string) {
    try {
      ...
      activeAttemptIsEditingRef.current = data.upaya?.status !== 'berlangsung'
  ...
  function applyWorkspaceData(data: Row, focusQuestionId?: string) {
    setWorkspace(data)
    const nextAnswers: Record<string, any> = {}
    const nextFlags: Record<string, boolean> = {}
    for (const question of data.soal || []) {
      nextAnswers[question.upayaSoalId] = parse(question.jawaban, null)
      nextFlags[question.upayaSoalId] = Boolean(question.ditandai)
    }
    const visible = new Set((data.soal || []).map((question: Row) => question.upayaSoalId))
    let discardedPending = false
    pendingAnswersRef.current.forEach((_, id) => { if (!visible.has(id)) { pendingAnswersRef.current.delete(id); discardedPending = true } })
    pendingFlagsRef.current.forEach((_, id) => { if (!visible.has(id)) { pendingFlagsRef.current.delete(id); discardedPending = true } })
    pendingAnswersRef.current.forEach((pending, id) => { if (visible.has(id)) nextAnswers[id] = pending.value })
    pendingFlagsRef.current.forEach((pending, id) => { if (visible.has(id)) nextFlags[id] = pending.value })
    setAnswers(nextAnswers)
    setFlagged(nextFlags)
    const focusedIndex = focusQuestionId ? (data.soal || []).findIndex((question: Row) => question.upayaSoalId === focusQuestionId) : -1
    setIndex(focusedIndex >= 0 ? focusedIndex : Math.min(index, Math.max(0, (data.soal?.length || 1) - 1)))
    persistPending(data.upaya?.id || activeAttemptRef.current)
    if (discardedPending) toast.info('Bagian yang dilewati diperbarui. Jawaban yang belum tersimpan pada bagian tersebut tidak ikut dikirim.')
  }
    ...
    const sync = (async () => {
      ...
      try {
        while (pendingAnswersRef.current.size || pendingFlagsRef.current.size) {
          ...
          const answer = pendingAnswersRef.current.entries().next().value as [string, PendingAnswer] | undefined
          if (answer) {
            ...
            const value = question?.soal?.tipe === 'unggah_berkas' && Array.isArray(pending.value)
              ...
              : pending.value
            const editingSubmittedResponse = activeAttemptIsEditingRef.current
            if (editingSubmittedResponse && !pending.idempotencyKey) {
              pending.idempotencyKey = newIdempotencyKey()
              pendingAnswersRef.current.set(questionId, pending)
              persistPending(attemptId)
            }
            const answerPath = editingSubmittedResponse
              ? `/simulasi/saya/upaya/${attemptId}/jawaban/${questionId}/revisi`
              : `/simulasi/saya/upaya/${attemptId}/jawaban/${questionId}`
            await sharedRequest(answerPath, 'PUT', editingSubmittedResponse ? { jawaban: value, idempotencyKey: pending.idempotencyKey } : { jawaban: value })
            if (activeAttemptRef.current !== attemptId) return false
            if (pendingAnswersRef.current.get(questionId)?.sequence === pending.sequence) pendingAnswersRef.current.delete(questionId)
            persistPending(attemptId)
            if (question?.soal?.hasBranching) {
              setSaveState('Memperbarui alur bagian…')
              const focusQuestionId = workspace?.soal?.[index]?.upayaSoalId
              const updatedWorkspace = await sharedRequest(`/simulasi/saya/upaya/${attemptId}`)
              if (activeAttemptRef.current !== attemptId) return false
              applyWorkspaceData(updatedWorkspace, focusQuestionId)
            }
            continue
          }
  ...
  async function saveAnswer(question: Row, value: any) {
    ...
    pendingAnswersRef.current.set(question.upayaSoalId, { value, sequence: answerSequenceRef.current, ...(workspace.upaya?.status !== 'berlangsung' ? { idempotencyKey: newIdempotencyKey() } : {}) })
  ...
  async function uploadAnswerFile(question: Row, file: File): Promise<Row> {
    ...
    const attemptId = String(workspace.upaya.id)
    const storageKey = `simulasi-file-upload:${attemptId}:${question.upayaSoalId}:${file.name}:${file.size}:${file.lastModified}`
    const idempotencyKey = persistedIdempotencyKey(storageKey)
    const form = new FormData()
    form.append('file', file)
    const response = await fetch(`${apiBase}/simulasi/saya/upaya/${workspace.upaya.id}/file/${question.upayaSoalId}`, {
      method: 'POST', credentials: 'include', headers: { Authorization: `Bearer ${token}`, ...(shareToken ? { 'X-Simulasi-Share-Token': shareToken } : {}), 'Idempotency-Key': idempotencyKey }, body: form,
    })
    const result = await response.json().catch(() => ({}))
    if (!response.ok) throw new Error(result?.error || result?.message || `Unggah gagal (${response.status}).`)
    clearPersistedIdempotencyKey(storageKey)
    setSaveState('Tersimpan')
    return result as Row
  }
  async function removeAnswerFile(fileId: string) {
    if (!workspace) throw new Error('Upaya pengerjaan tidak tersedia.')
    const storageKey = `simulasi-file-delete:${workspace.upaya.id}:${fileId}`
    const idempotencyKey = persistedIdempotencyKey(storageKey)
    await sharedRequest(`/simulasi/saya/upaya/${workspace.upaya.id}/file/${fileId}`, 'DELETE', undefined, { 'Idempotency-Key': idempotencyKey })
    clearPersistedIdempotencyKey(storageKey)
    setSaveState('Tersimpan')
  }
  ...
  async function showAttemptResult(attemptId: string, warnAboutLocalAnswers = false) {
    ...
    activeAttemptIsEditingRef.current = false
  ...
  async function submit(confirmed = false, timedOut = false) {
    ...
    const editingSubmittedResponse = workspace.upaya?.status !== 'berlangsung'
    const reachedDeadline = !editingSubmittedResponse && (timedOut || remainingRef.current <= 0)
    submittingRef.current = true
    setSubmitting(true)
    try {
      window.clearTimeout(saveTimerRef.current)
      const synced = await flushAnswerQueue(workspace.upaya.id)
      if (editingSubmittedResponse) {
        if (!synced || pendingAnswersRef.current.size || pendingFlagsRef.current.size) throw new Error('Perubahan belum tersimpan. Sambungkan internet dan coba lagi.')
  ...
  const optionImageIDs = new Set((current?.soal?.konfigurasi?.choices || []).map((choice: Row) => String(choice.imageId || '')).filter(Boolean))
  const standaloneStimulus = (current?.soal?.stimulus || []).filter((item: Row) => !optionImageIDs.has(String(item.id || '')))
  const answeredCount = questions.filter((question: Row) => answered(question, answers[question.upayaSoalId])).length
  const progress = questions.length ? answeredCount / questions.length * 100 : 0
  const currentFlagged = Boolean(current && flagged[current.upayaSoalId])
  const sectionQuestions = current?.bagianId ? questions.filter((question: Row) => question.bagianId === current.bagianId) : []
  const sectionAnswered = sectionQuestions.filter((question: Row) => answered(question, answers[question.upayaSoalId])).length

  if (loading) return <Centered label="Memuat simulasi kamu…" />
  if (result) return <ResultScreen result={result} onBack={() => { setResult(null); void load() }} onEdit={() => { if (result.upayaId) void openWorkspace(String(result.upayaId)) }} />
  if (workspace && current) {
    const editingSubmittedResponse = workspace.upaya?.status !== 'berlangsung'
    const urgent = !editingSubmittedResponse && remaining < 300
    return <main data-assessment-workspace="student-exam" className="min-h-screen bg-[#edf3fa] text-slate-900">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-gradient-to-r from-[#1c5d94] via-[#287db7] to-[#24527d] text-white shadow-lg">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-3 py-3 sm:px-5">
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">{editingSubmittedResponse ? <Badge className="border border-white/25 bg-white/15 px-3 py-2 text-white">Mode revisi respons</Badge> : <div className={`flex items-center gap-2 rounded-xl px-3 py-2 font-mono text-base font-bold shadow-inner sm:text-xl ${urgent ? 'bg-red-500/90 text-white' : 'bg-white/15 text-white'}`} role="timer" aria-live="off" aria-label={`Sisa waktu ${clock(remaining)}`}><Clock3 className="h-4 w-4 sm:h-5 sm:w-5" aria-hidden="true" />{clock(remaining)}</div>}<Button size="icon" variant="ghost" className="h-11 w-11 min-h-11 min-w-11 text-white hover:bg-white/15" onClick={onLogout} aria-label="Keluar dari portal"><LogOut className="h-4 w-4" /></Button></div>
        </div>
      </header>
      <div className="mx-auto w-full max-w-[1500px] space-y-3 px-3 py-3 sm:px-5 sm:py-4">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-3 shadow-sm sm:px-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-1.5"><span className="mr-1 hidden text-xs text-slate-500 sm:inline">Ukuran teks</span>{(['small', 'medium', 'large'] as FontScale[]).map((size, sizeIndex) => <Button key={size} size="icon" variant={fontScale === size ? 'default' : 'outline'} className="h-11 w-11 min-h-11 min-w-11 rounded-lg" onClick={() => setFontScale(size)} aria-label={`Ukuran teks ${size === 'small' ? 'kecil' : size === 'large' ? 'besar' : 'sedang'}`} aria-pressed={fontScale === size}><span className={sizeIndex === 0 ? 'text-xs' : sizeIndex === 1 ? 'text-sm' : 'text-base'}>A</span></Button>)}<Button size="sm" variant="outline" className="min-h-11 min-w-11" onClick={(event) => { infoTriggerRef.current = event.currentTarget; setShowInfo(true) }} aria-label="Informasi soal"><Info className="h-4 w-4" /> <span className="hidden sm:inline">Informasi soal</span></Button><Button size="sm" variant="outline" className="min-h-11 xl:hidden" onClick={() => setShowPalette(true)}><Grid2X2 className="h-4 w-4" /> Daftar soal</Button></div>
        </div>
        {editingSubmittedResponse && <div role="status" className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">Kamu sedang memperbarui respons yang sudah dikirim. Setiap perubahan disimpan sebagai revisi. {workspace.paket?.waktuSelesai ? `Perubahan tersedia sampai ${new Date(workspace.paket.waktuSelesai).toLocaleString('id-ID')}.` : ''}</div>}
        ...
        {current?.namaBagian && <Card className="flex flex-wrap items-center justify-between gap-3 border-[#2f8cca]/25 bg-white px-4 py-3 shadow-sm"><div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-[.14em] text-[#1d6fa8]">Bagian {current.urutanBagian || ''}</p><h2 className="font-bold text-slate-900">{current.namaBagian}</h2>{current.deskripsiBagian && <p className="mt-1 whitespace-pre-wrap text-sm text-slate-600">{current.deskripsiBagian}</p>}</div><Badge variant="secondary" className="shrink-0">{sectionAnswered} dari {sectionQuestions.length} terjawab</Badge></Card>}
        <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(380px,1fr)_248px]">
          <Card className="min-h-[480px] overflow-hidden border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3"><div><p className="text-[11px] font-bold uppercase tracking-[.14em] text-[#1d6fa8]">Stimulus</p><h2 className="font-semibold">Bacaan / informasi pendukung</h2></div><span className="text-xs text-slate-500">Panel dapat digulir</span></div><div className="max-h-[calc(100vh-285px)] min-h-[390px] overflow-y-auto p-4 sm:p-6" style={{ fontSize: `${scalePx(fontScale)}px` }}><StimulusContent items={standaloneStimulus} token={token} shareToken={shareToken} emptyLabel="Soal ini tidak menggunakan stimulus tambahan." /></div></Card>
          <Card className="min-h-[480px] overflow-hidden border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3"><div><p className="text-[11px] font-bold uppercase tracking-[.14em] text-[#1d6fa8]">Pertanyaan {index + 1}</p><p className="text-xs text-slate-500">Pilih atau tulis jawabanmu dengan teliti.</p></div>{!editingSubmittedResponse && <Button size="sm" variant={currentFlagged ? 'default' : 'outline'} className="h-9" onClick={() => void toggleFlag(current)} aria-pressed={currentFlagged}><Flag className={`h-3.5 w-3.5 ${currentFlagged ? 'fill-current' : ''}`} />{currentFlagged ? 'Ditandai' : 'Ragu-ragu'}</Button>}</div><div className="flex min-h-[390px] flex-col p-4 sm:p-6" style={{ fontSize: `${scalePx(fontScale)}px` }}><p className="mb-6 whitespace-pre-wrap text-[1.08em] font-semibold leading-relaxed" id={`question-${current.upayaSoalId}`}>{current.soal?.pertanyaan}</p><QuestionAnswerControl question={current.soal || { tipe: 'isian_singkat' }} questionId={`question-${current.upayaSoalId}`} value={answers[current.upayaSoalId]} onChange={(value) => current.soal?.tipe === 'unggah_berkas' ? setAnswers((previous) => ({ ...previous, [current.upayaSoalId]: value })) : void saveAnswer(current, value)} fontScale={fontScale} token={token} shareToken={shareToken} onFileUpload={(file) => uploadAnswerFile(current, file)} onFileRemove={(fileId) => removeAnswerFile(fileId)} onFileDownload={(fileId, name) => void downloadAnswerFile(fileId, name)} />{editingSubmittedResponse && current.soal?.tipe === 'unggah_berkas' && <p className="mt-2 text-xs text-amber-700">Setiap perubahan lampiran tersimpan sebagai revisi dan meminta guru menilai ulang.</p>}<div className="mt-auto flex flex-wrap justify-between gap-2 border-t border-slate-100 pt-5"><Button variant="outline" className="min-h-11" disabled={index === 0} onClick={() => setIndex((value) => value - 1)}><ArrowLeft className="h-4 w-4" /> Sebelumnya</Button>{index === questions.length - 1 ? <Button className="min-h-11" onClick={() => void submit()}><Send className="h-4 w-4" />{editingSubmittedResponse ? 'Simpan perubahan' : 'Kirim simulasi'}</Button> : <Button className="min-h-11" onClick={() => setIndex((value) => value + 1)}>Berikutnya <ArrowRight className="h-4 w-4" /></Button>}</div></div></Card>
          <QuestionPalette questions={questions} index={index} answers={answers} flagged={flagged} onSelect={setIndex} onSubmit={() => void submit()} editing={editingSubmittedResponse} />
        </div>
      </div>
      {showPalette && <Modal title="Daftar soal" onClose={() => setShowPalette(false)}><QuestionPalette questions={questions} index={index} answers={answers} flagged={flagged} onSelect={(next) => { setIndex(next); setShowPalette(false) }} onSubmit={() => { setShowPalette(false); void submit() }} editing={editingSubmittedResponse} mobile /></Modal>}
      {showInfo && <Modal title="Informasi pengerjaan" onClose={() => setShowInfo(false)} returnFocusRef={infoTriggerRef}><div className="space-y-3 text-sm leading-relaxed text-slate-600"><p>Jawaban tersimpan otomatis saat kamu memilih atau mengetik. Pastikan status penyimpanan menunjukkan <strong className="text-slate-900">Tersimpan</strong> sebelum berpindah soal.</p><ul className="list-disc space-y-1 pl-5"><li>Gunakan tombol <strong className="text-slate-900">Ragu-ragu</strong> untuk menandai soal yang ingin diperiksa.</li><li>Gunakan tombol ukuran teks agar nyaman dibaca.</li><li>Panah kiri/kanan pada keyboard dapat digunakan untuk berpindah soal.</li><li>Pengiriman tidak dapat dibatalkan setelah dikonfirmasi.</li></ul></div></Modal>}
      {confirmSubmit && <Modal title={editingSubmittedResponse ? 'Periksa perubahan respons' : 'Periksa sebelum mengirim'} onClose={() => setConfirmSubmit(false)}><SubmitSummary questions={questions} answers={answers} flagged={flagged} /><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button variant="outline" onClick={() => setConfirmSubmit(false)}>Kembali memeriksa</Button><Button disabled={submitting} onClick={() => void submit(true)}>{submitting ? 'Menyimpan…' : editingSubmittedResponse ? 'Simpan revisi' : 'Ya, kirim simulasi'}</Button></div></Modal>}
    </main>
  }
  if (instruction) return <InstructionScreen instruction={instruction} onStart={() => void start()} onBack={() => setInstruction(null)} />
  return <main data-assessment-workspace="student-portal" className="min-h-screen bg-[#edf3fa]"><header className="relative overflow-hidden bg-gradient-to-r from-[#1c5d94] via-[#287db7] to-[#24527d] text-white shadow-lg"><div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3 px-4 py-7 sm:px-8 sm:py-9"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-full bg-white/15 text-xl font-black ring-1 ring-white/35">TI</div><div><p className="text-xs font-bold uppercase tracking-[.18em] text-white/75">Portal Peserta Didik</p><h1 className="text-2xl font-black sm:text-3xl">Simulasi ANBK / TKA SD</h1><p className="mt-1 text-sm text-white/75">Latihan terarah dengan pengalaman ujian yang nyaman.</p></div></div><Button variant="outline" className="border-white/40 bg-white/10 text-white hover:bg-white/20" onClick={onLogout}><LogOut className="h-4 w-4" /> Keluar</Button></div></header><div className="mx-auto max-w-5xl px-4 pb-8 sm:px-8">{shareToken && <div className="relative z-10 -mt-4 mb-4 flex items-start gap-2 rounded-xl border border-[#2f8cca]/25 bg-white p-3 text-sm text-[#1d5f8e] shadow-sm"><Link2 className="mt-0.5 h-4 w-4 shrink-0" /><span>Kamu membuka tautan simulasi bersama. Kerjakan hanya jika tautan diberikan oleh guru.</span></div>}<div className="grid gap-4 sm:grid-cols-2">{rows.map((row) => { const packet = row.paket || {}; const attempts: Row[] = row.attempts || []; const ongoing = attempts.find((attempt) => attempt.status === 'berlangsung'); const finished = attempts.filter((attempt) => attempt.status !== 'berlangsung'); const atLimit = attempts.length >= Number(packet.maksPercobaan || 1); return <Card key={packet.id} className="group overflow-hidden border-slate-200 p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex items-center justify-between gap-2"><Badge variant={packet.mode === 'tka_sd' ? 'secondary' : 'default'}>{packet.mode === 'tka_sd' ? 'Simulasi TKA SD' : 'Simulasi ANBK/AKM'}</Badge>{ongoing && <span className="text-xs font-semibold text-amber-700">Sedang berlangsung</span>}</div><h2 className="mt-3 text-lg font-bold">{packet.nama}</h2><p className="mt-1 min-h-10 text-sm leading-relaxed text-muted-foreground">{packet.deskripsi || 'Latihan komputer untuk menguatkan kemampuanmu.'}</p><div className="mt-4 flex flex-wrap gap-2 text-xs text-muted-foreground"><span className="rounded-lg bg-slate-100 px-2.5 py-1">{packet.durasiMenit} menit</span><span className="rounded-lg bg-slate-100 px-2.5 py-1">Maks. {packet.maksPercobaan} percobaan</span></div><Button className="mt-5 min-h-11 w-full" disabled={!row.tersedia} onClick={() => ongoing ? void openWorkspace(ongoing.id) : void openInstruction(packet)}>{ongoing ? <><Play className="h-4 w-4" /> Lanjutkan pengerjaan</> : row.tersedia ? <><Play className="h-4 w-4" /> Lihat instruksi</> : atLimit ? 'Batas percobaan tercapai' : 'Belum tersedia'}</Button>{finished.length > 0 && <section className="mt-5 border-t border-slate-100 pt-4" aria-label={`Riwayat hasil ${packet.nama}`}><h3 className="mb-2 text-sm font-semibold text-slate-700">Riwayat pengerjaan</h3><div className="space-y-2">{finished.map((attempt) => <Button key={attempt.id} variant="outline" className="min-h-11 w-full justify-between" onClick={() => void openResult(attempt.id)}><span>Lihat hasil · Percobaan {attempt.nomor}</span><span className="text-xs text-muted-foreground">{attempt.status === 'menunggu_nilai' ? 'Menunggu penilaian' : attempt.skor !== undefined ? `Nilai ${Number(attempt.skor).toFixed(1)}` : 'Selesai'}</span></Button>)}</div></section>}</Card> })}{!rows.length && <Card className="p-12 text-center sm:col-span-2"><Play className="mx-auto h-10 w-10 text-[#2f8cca]/50" /><h2 className="mt-3 font-bold">Belum ada simulasi</h2><p className="text-sm text-muted-foreground">Guru akan menugaskan simulasi kepadamu di sini.</p></Card>}</div></div></main>
}

function QuestionPalette({ questions, index, answers, flagged, onSelect, onSubmit, editing = false, mobile = false }: { questions: Row[]; index: number; answers: Record<string, any>; flagged: Record<string, boolean>; onSelect: (index: number) => void; onSubmit: () => void; editing?: boolean; mobile?: boolean }) {
  const unanswered = questions.length - questions.filter((question) => answered(question, answers[question.upayaSoalId])).length
  return <Card className={`${mobile ? '' : 'hidden xl:block'} h-fit border-slate-200 p-4 shadow-sm`}><div className="flex items-center justify-between"><div><h2 className="text-sm font-bold">Daftar soal</h2><p className="text-[11px] text-slate-500">Pilih nomor untuk berpindah</p></div><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-600">{unanswered} kosong</span></div><div className="mt-4 grid grid-cols-5 gap-2">{questions.map((question: Row, questionIndex: number) => {
    const hasAnswer = answered(question, answers[question.upayaSoalId])
    return <button type="button" key={question.upayaSoalId} onClick={() => onSelect(questionIndex)} className={`relative min-h-11 min-w-11 rounded-lg text-sm font-bold outline-none ring-offset-2 transition focus-visible:ring-2 focus-visible:ring-[#1d6fa8] ${questionIndex === index ? 'bg-[#1d6fa8] text-white shadow-sm' : hasAnswer ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`} aria-label={`Buka soal ${questionIndex + 1}${hasAnswer ? ', sudah dijawab' : ', belum dijawab'}${flagged[question.upayaSoalId] ? ', ditandai' : ''}`}>{questionIndex + 1}{flagged[question.upayaSoalId] && <Flag className="absolute -right-1 -top-1 h-3 w-3 fill-amber-400 text-amber-600" aria-hidden="true" />}</button>
  })}</div><div className="mt-4 space-y-1 text-[11px] text-slate-500"><p><span className="mr-1 inline-block h-2 w-2 rounded-full bg-[#1d6fa8]" />Sedang dibuka</p><p><span className="mr-1 inline-block h-2 w-2 rounded-full bg-emerald-400" />Sudah dijawab</p>{!editing && <p><Flag className="mr-1 inline h-3 w-3 text-amber-600" />Ragu-ragu</p>}</div><Button className="mt-5 min-h-11 w-full" variant="outline" onClick={onSubmit}><Send className="h-4 w-4" /> {editing ? 'Simpan perubahan' : 'Selesai & kirim'}</Button></Card>
}
function SubmitSummary({ questions, answers, flagged }: { questions: Row[]; answers: Record<string, any>; flagged: Record<string, boolean> }) {
  const unanswered = questions.filter((question) => !answered(question, answers[question.upayaSoalId])).length
  const review = questions.filter((question) => flagged[question.upayaSoalId]).length
  return <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-3"><div className="rounded-xl bg-emerald-50 p-3"><strong className="block text-2xl text-emerald-700">{questions.length - unanswered}</strong><span className="text-xs text-emerald-800">Terjawab</span></div><div className="rounded-xl bg-amber-50 p-3"><strong className="block text-2xl text-amber-700">{review}</strong><span className="text-xs text-amber-800">Ragu-ragu</span></div><div className="rounded-xl bg-red-50 p-3"><strong className="block text-2xl text-red-700">{unanswered}</strong><span className="text-xs text-red-800">Kosong</span></div></div>
}
function Modal({ title, onClose, children, returnFocusRef }: { title: string; onClose: () => void; children: ReactNode; returnFocusRef?: { current: HTMLElement | null } }) { return <Dialog open onOpenChange={(open) => { if (!open) onClose() }}><DialogContent onCloseAutoFocus={(event) => { if (!returnFocusRef) return; event.preventDefault(); requestAnimationFrame(() => returnFocusRef.current?.focus()) }} className="max-h-[90vh] w-full max-w-lg overflow-auto rounded-2xl p-5 shadow-2xl sm:p-5"><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription className="sr-only">Dialog berisi informasi atau konfirmasi terkait pengerjaan simulasi.</DialogDescription></DialogHeader>{children}</DialogContent></Dialog> }
function InstructionScreen({ instruction, onStart, onBack }: { instruction: Row; onStart: () => void; onBack: () => void }) { const packet = instruction.paket || {}; return <main data-assessment-workspace="student-instructions" className="min-h-screen bg-[#edf3fa]"><header className="bg-gradient-to-r from-[#1c5d94] via-[#287db7] to-[#24527d] text-white"><div className="mx-auto max-w-2xl px-5 py-8 sm:px-8"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-full bg-white/15 text-xl font-black ring-1 ring-white/35">TI</div><div><p className="text-xs font-bold uppercase tracking-[.18em] text-white/75">Portal Peserta Didik</p><p className="text-sm text-white/80">Simulasi ANBK / TKA SD</p></div></div></div></header><div className="relative z-10 mx-auto -mt-10 max-w-2xl px-4 pb-8"><Card className="overflow-hidden border-slate-200 shadow-xl"><div className="border-b border-slate-200 p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#1d6fa8]">Konfirmasi simulasi</p><h1 className="mt-2 text-2xl font-black text-slate-900 sm:text-3xl">{packet.nama}</h1><p className="mt-2 text-sm text-muted-foreground">Pastikan kamu siap sebelum memulai. Waktu dihitung setelah tombol mulai ditekan.</p></div><div className="p-6 sm:p-8"><div className="grid grid-cols-3 gap-2 rounded-2xl bg-slate-50 p-4 text-center text-sm"><div><strong className="block text-xl text-[#1d6fa8]">{instruction.jumlahSoal}</strong>soal</div><div><strong className="block text-xl text-[#1d6fa8]">{packet.durasiMenit}</strong>menit</div><div><strong className="block text-xl text-[#1d6fa8]">{packet.maksPercobaan}</strong>percobaan</div></div><h2 className="mt-6 font-bold">Petunjuk pengerjaan</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-muted-foreground">{packet.instruksi || 'Baca soal dengan teliti. Jawabanmu tersimpan otomatis.'}</p><ul className="mt-4 list-disc space-y-1 pl-5 text-sm leading-relaxed text-muted-foreground"><li>Gunakan daftar soal untuk berpindah dengan cepat.</li><li>Tandai soal ragu-ragu agar mudah diperiksa kembali.</li><li>Gunakan kontrol ukuran teks sesuai kenyamanan membaca.</li><li>Pengiriman tidak dapat dibatalkan setelah dikonfirmasi.</li></ul><div className="mt-8 flex flex-col-reverse gap-2 sm:flex-row sm:justify-between"><Button variant="outline" className="min-h-11" onClick={onBack}>Kembali</Button><Button className="min-h-11 sm:min-w-48" onClick={onStart}><Play className="h-4 w-4" /> Mulai simulasi</Button></div></div></Card></div></main> }
function ResultScreen({ result, onBack, onEdit }: { result: Row; onBack: () => void; onEdit: () => void }) { const summary = result.ringkasan; return <main data-assessment-workspace="student-result" className="min-h-screen bg-[#edf3fa]"><header className="bg-gradient-to-r from-[#1c5d94] via-[#287db7] to-[#24527d] text-white"><div className="mx-auto max-w-lg px-5 py-8 sm:px-8"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-full bg-white/15 text-xl font-black ring-1 ring-white/35">TI</div><div><p className="text-xs font-bold uppercase tracking-[.18em] text-white/75">Portal Peserta Didik</p><p className="text-sm text-white/80">Hasil simulasi</p></div></div></div></header><div className="relative z-10 mx-auto -mt-10 max-w-lg px-4 pb-8"><Card className="border-slate-200 p-8 text-center shadow-xl"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-emerald-100"><CheckCircle2 className="h-9 w-9 text-emerald-600" /></div><h1 className="mt-4 text-2xl font-bold">Simulasi selesai</h1><p className="mt-2 text-sm text-muted-foreground">{result.status === 'menunggu_nilai' ? 'Jawaban uraianmu menunggu penilaian guru.' : result.pesanKonfirmasi || 'Jawabanmu sudah berhasil dikirim.'}</p>{result.skor !== undefined && <div className="mt-6 rounded-2xl bg-[#eaf5fd] p-5"><p className="text-sm text-[#1d5f8e]">Nilai kamu</p><strong className="text-4xl text-[#1d6fa8]">{Number(result.skor).toFixed(1)}</strong></div>}{summary && <div className="mt-4 grid grid-cols-3 gap-2 text-sm"><div className="rounded-xl bg-emerald-50 p-3"><strong className="block text-lg text-emerald-700">{summary.benar}</strong>Benar</div><div className="rounded-xl bg-red-50 p-3"><strong className="block text-lg text-red-700">{summary.salah}</strong>Salah</div><div className="rounded-xl bg-slate-100 p-3"><strong className="block text-lg">{summary.kosong}</strong>Kosong</div></div>}{(result.pembahasan || []).length > 0 && <div className="mt-5 space-y-3 text-left"><h2 className="font-bold">Pembahasan</h2>{result.pembahasan.map((item: Row) => <div key={item.urutan} className="rounded-xl bg-muted/50 p-3 text-sm"><p className="font-medium">{item.urutan}. {item.pertanyaan}</p><p className="mt-1 whitespace-pre-wrap text-muted-foreground">{item.pembahasan || 'Belum ada pembahasan.'}</p></div>)}</div>}{result.bolehEditRespons && <Button className="mt-7 min-h-11 w-full" variant="outline" onClick={onEdit}>Ubah respons</Button>}<Button className="mt-3 min-h-11 w-full" onClick={onBack}>Kembali ke daftar</Button></Card></div></main> }
