...
import { ArrowDown, ArrowUp, CheckCircle2, CircleDot, ClipboardCheck, Copy, Download, ExternalLink, FileText, FileUp, GripVertical, ImageIcon, KeyRound, Link2, ListChecks, Plus, Save, Send, Table2, TextCursorInput, Trash2, UsersRound, Wifi, WifiOff, X } from 'lucide-react'
...
export function SimulasiView({ token, user }: { token: string; user: User }) {
  ...
  const load = () => {
    void Promise.all([
      request('/simulasi/soal', token), request('/bank-soal', token).catch(() => []), request('/simulasi/paket', token), request('/simulasi/bahan', token).catch(() => []), request('/mapel', token), request('/kelas', token).catch(() => []), request('/peserta-didik', token).catch(() => []),
  ...
  async function createShareToken() {
    if (!selectedPackage) return
    try {
      const created = await request(`/simulasi/paket/${selectedPackage.id}/share-token`, token, 'POST', { label: 'Tautan siswa' })
    ...
    } catch (error: any) { toast.error(error.message || 'Tautan belum dapat dibuat.') }
  }
...
export function SimulasiPreviewView({ token }: { token: string }) {
  ...
  if (error) return <main className="grid min-h-screen place-items-center bg-slate-100 p-6"><Card className="max-w-md p-6 text-center"><h1 className="font-bold">Pratinjau tidak tersedia</h1><p className="mt-2 text-sm text-muted-foreground">{error}</p><Button className="mt-5" onClick={() => window.close()}>Tutup tab</Button></Card></main>
...
function PackageDetail({ packet, items, questions, classes, students, assigned, setAssigned, results, shareTokens, detail, writable, canGrade, onAttach, onAssign, onPublish, onDuplicate, onInspect, onGrade, onCreateShare, onRevokeShare, onPreview, onExport, onExportXlsx }: any) {
  return <div className="space-y-4">
    <div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-bold">{packet.nama}</h3><p className="text-xs text-muted-foreground">{items.length} soal · {assigned.length} peserta · status {packet.status}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={onPreview}><ExternalLink className="h-3.5 w-3.5" /> Pratinjau</Button><Button size="sm" variant="outline" onClick={onDuplicate}><Copy className="h-3.5 w-3.5" /> Duplikasi</Button>{results.length > 0 && <><Button size="sm" variant="outline" onClick={onExport}><Download className="h-3.5 w-3.5" /> CSV</Button><Button size="sm" variant="outline" onClick={onExportXlsx}><Download className="h-3.5 w-3.5" /> XLSX</Button></>}{writable && <Button size="sm" onClick={onPublish}><Send className="h-3.5 w-3.5" /> Terbitkan</Button>}</div></div>
    {packet.status === 'terbit' && <ShareTokenPanel tokens={shareTokens || []} canManage={canGrade} onCreate={onCreateShare} onRevoke={onRevokeShare} />}
...
function ShareTokenPanel({ tokens, canManage, onCreate, onRevoke }: { tokens: Row[]; canManage: boolean; onCreate: () => void; onRevoke: (id: string) => void }) {
  ...
  return <section className="rounded-xl border border-primary/20 bg-primary/[0.03] p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><h4 className="flex items-center gap-2 text-sm font-semibold"><KeyRound className="h-4 w-4 text-primary" />Tautan pengerjaan siswa</h4><p className="mt-1 text-xs text-muted-foreground">Buat link aman. Siswa tetap harus masuk dengan akun siswa.</p></div>{canManage && <Button size="sm" onClick={onCreate}><Link2 className="h-3.5 w-3.5" />Buat tautan</Button>}</div>{tokens.length > 0 && <div className="mt-3 space-y-2">{tokens.map((row) => <div key={row.id} className="flex flex-wrap items-center gap-2 rounded-lg border bg-white p-2 text-xs"><span className="font-mono text-muted-foreground">…{row.tokenPrefix}</span><Badge variant={row.status === 'aktif' ? 'default' : 'secondary'}>{row.status}</Badge>{row.expiresAt && <span className="text-muted-foreground">s.d. {new Date(row.expiresAt).toLocaleDateString('id-ID')}</span>}{row.url ? <Input className="min-w-0 w-full flex-1 font-mono text-[11px] sm:min-w-[220px]" readOnly value={row.url} onFocus={(event) => event.currentTarget.select()} aria-label="Tautan pengerjaan" /> : <span className="text-muted-foreground">Link hanya dapat disalin saat dibuat.</span>}<div className="ml-auto flex gap-1">{row.url && <Button size="sm" variant="outline" onClick={() => void copy(row.url)}><Copy className="h-3.5 w-3.5" />Salin</Button>}{canManage && row.status === 'aktif' && <Button size="sm" variant="outline" onClick={() => onRevoke(row.id)}>Cabut</Button>}</div></div>)}</div>}{!tokens.length && <p className="mt-3 text-xs text-muted-foreground">Belum ada tautan aktif.</p>}</section>
}
